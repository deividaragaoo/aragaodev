...
export function VirtualAgent({ hidden = false }: VirtualAgentProps) {
  ...
  const messages = t.agent.messages;
  const whatsappUrl = `${contact.whatsapp.url}?text=${encodeURIComponent(t.agent.whatsappMessage)}`;
  ...
  }, [visible, minimized, messageIndex, hidden, messages]);
  ...
  }, [visible, minimized, hidden, messages.length]);
      ...
      {visible && (
        ...
        >
              ...
              >
                <button
                  ...
                  aria-label={t.agent.minimize}
                    className="absolute -top-2 -right-2 z-10 flex h-8 w-8 sm:h-9 sm:w-9 items-center justify-center rounded-full border border-border bg-elevated text-muted hover:text-foreground transition-colors"
                >
                ...
                <a
                  ...
                  className="group relative block rounded-2xl border border-border bg-elevated p-3.5 sm:p-4 shadow-[0_16px_48px_rgba(0,0,0,0.18)] transition-colors duration-300 hover:border-[#25D366]/30 active:scale-[0.99]"
                >
                  ...
                  <span className="mt-2.5 sm:mt-3 inline-flex items-center gap-1.5 text-[11px] font-medium text-[#34eb7a] group-hover:text-[#25D366] transition-colors">
                    ...
                    <span className="hidden sm:inline">{t.agent.whatsapp}</span>
                    <span className="sm:hidden">WhatsApp</span>
                  </span>

                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute -bottom-1.5 right-5 h-3 w-3 rotate-45 border-r border-b border-border bg-elevated"
                  />
                </a>
              </motion.div>
            )}
          ...
          <motion.button
            ...
            aria-label={minimized ? t.agent.open : t.agent.close}
            className={cn(
              "pointer-events-auto relative flex h-12 w-12 sm:h-14 sm:w-14 items-center justify-center rounded-full border border-border bg-elevated shadow-[0_12px_40px_rgba(0,0,0,0.18)] transition-all duration-300 hover:border-accent-orange/40 active:scale-95",
          ...
          >
            ...
            {minimized && (
              <span className="absolute top-0 right-0 h-2.5 w-2.5 sm:h-3 sm:w-3 rounded-full border-2 border-elevated bg-[#25D366]" />
            )}
          </motion.button>
        </div>
      )}
